...
use cosmwasm_std::{
    attr, from_binary, to_json_binary, Addr, BankMsg, Binary, Coin, CosmosMsg, CustomQuery,
    Decimal, Deps, DepsMut, Empty, Env, MessageInfo, QueryRequest, QueryResponse, Response,
    StdError, StdResult, SubMsg, Uint128, WasmMsg, WasmQuery, Reply
};
...
use crate::state::{
    Deposits, Epoch, ADMIN, CHECK_POINTED_EPOCH_ID, CONFIG, EPOCH, EPOCH_DURATION, FUND_INFO,
    USER_DEPOSIT,
};
use crate::ContractResult;
use crate::{error::ContractError, state::WRITE_DOWNS_BY_POOL_AMOUNT};
use crate::{
    msg::{DepositMsg, ExecuteMsg, InstantiateMsg, QueryMsg},
    state::Config,
};
use tranche_pool::{self, msg::QueryMsg as TranchePoolQueryMsg};
...
pub fn instantiate(
    ...
    let nft_token_isser = deps.api.addr_validate(&msg.nft_token_addr)?;

    let config = Config {
        lp_token: String::new(),
        nft_token_issuer: msg.nft_token_addr,
        pool_denom: msg.pool_denom,
        max_leverage_ratio: msg.max_leverage_ratio,
        junior_pools: msg.junior_pools,
    };
    ...
    let submsg = SubMsg::reply_on_success(cosmos_msg, INSTANTIATE_REPLY_ID);
...
pub fn handle_instantiate_reply(deps: DepsMut, msg: Reply) -> StdResult<Response> {
    ...
    config.lp_token = res.contract_address.clone();
    CONFIG.save(deps.storage, &config)?;

    Ok(Response::new().add_attribute("lp_token_issuer", config.lp_token.to_string()))
}
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Invest {
            ...
            tranche_id,
            amount,
        } => invest(_deps, _env, _info, pool_id, tranche_id, amount),
    }
}
...
pub fn writedown(
    ...
    let query_nft = TranchePoolQueryMsg::GetInvestmentInfo { token_id };
    let query_msg = QueryRequest::Wasm(WasmQuery::Smart {
        contract_addr: config.junior_pools.clone(),
        msg: to_json_binary(&query_nft)?,
    });

    let investment_token = deps
        .querier
        .query::<cw721_metadata_onchain::InvestorToken>(&query_msg)?;

    let query_msg = QueryRequest::Wasm(WasmQuery::Smart {
        contract_addr: config.junior_pools,
        msg: to_json_binary(&TranchePoolQueryMsg::GetPoolInfo {
            id: investment_token.pool_id,
        })?,
    });
    let pool = deps
        .querier
        .query::<tranche_pool::msg::PoolInfo>(&query_msg)?;

    let mut token_info = investment_token.lend_info.clone();

    _apply_epoch_checkpoints(deps.branch(), env, info);
...
pub fn _calculate_writedown(
    principal_remaining: Uint128,
    pool: tranche_pool::msg::PoolInfo,
...
pub fn invest(
    ...
    let deposit_msg = tranche_pool::msg::ExecuteMsg::Deposit {
        msg: tranche_pool::msg::DepositMsg {
            amount,
            pool_id,
            tranche_id,
        },
...
pub fn _apply_epoch_checkpoints(
    deps: DepsMut,
    _env: Env,
    _info: MessageInfo,
) -> ContractResult<Response> {
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::GetUserDeposits { address } => {
            to_json_binary(&query_user_deposits(deps, address)?)
        }
        QueryMsg::GetConfig {} => to_json_binary(&CONFIG.load(deps.storage)?),
        QueryMsg::GetFundInfo {} => to_json_binary(&FUND_INFO.load(deps.storage)?),
        QueryMsg::MaxLeverageRatio {} => to_json_binary(&get_leverage_ratio(deps)?),
    }
}
